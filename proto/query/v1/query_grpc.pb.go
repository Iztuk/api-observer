...
func (c *logServiceClient) Query(ctx context.Context, in *LogParams, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Log], error) {
	...
	x := &grpc.GenericClientStream[LogParams, Log]{ClientStream: stream}
...
type LogService_QueryClient = grpc.ServerStreamingClient[Log]
...
func _LogService_Query_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(LogServiceServer).Query(m, &grpc.GenericServerStream[LogParams, Log]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type LogService_QueryServer = grpc.ServerStreamingServer[Log]
